...
fun Diagnostics.report(): String = buildString {
    ...
    with(settings) {
        appendLine("defaultTime=$defaultTime dayStart=$dayStart afternoon=$afternoon evening=$evening weekend=$weekendDay@$weekendTime")
        appendLine("actions=${defaultActions.joinToString("+") { it.name.take(2) }} sound=${alertSound.javaClass.simpleName}${insistentSound?.let { "/${it.javaClass.simpleName}" } ?: ""} plays=$soundPlays gap=${soundGapMinutes}m rounds=$soundRounds vibration=${vibration.strength}/${vibration.rhythm}")
...
internal fun Condition.diagLine(): String = when (this) {
    ...
    is Condition.Moving -> "moving >=${fixed(minMps, 1)}" + (maxMps?.let { "<${fixed(it, 1)}" } ?: "") + "m/s"
