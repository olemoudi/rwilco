...
import dev.rwilco.model.DEFAULT_PERSONAL_SLOTS
import dev.rwilco.model.DEFAULT_WORK_SLOTS
 ...
 * is where this could go wrong quietly: a contact reaching Home before its turn, or a row
 * reading its plazo instead of its turn and drawing a red track over somebody being paced.
 */
class ContactsWiringTest {

    private val zone = ZoneId.of("Europe/Madrid")

    /** A Monday, so the week's openings are all still ahead. */
    private val now: Instant = LocalDateTime.of(2026, 8, 31, 9, 0).atZone(zone).toInstant()
    private val dayStart = LocalTime.of(9, 0)

    private fun slots(kind: ContactKind): List<ContactSlot> =
        if (kind == ContactKind.WORK) DEFAULT_WORK_SLOTS else DEFAULT_PERSONAL_SLOTS

    private fun contact(id: String, kind: ContactKind = ContactKind.WORK, lastFiredAt: Instant? = null) = Reminder(
        id = id,
        text = id,
        recurrence = Recurrence.Since(8, RecurrenceUnit.WEEKS),
        contactKind = kind,
    ...
    fun `a contact waiting its turn is nowhere on Home`() {
        // Its plazo ran out months ago; the budget is pacing it, and that is not a complaint.
    ...
    @Test
    fun `a contact's row is about its turn, not its plazo`() {
        val state = buildRoutinesState(listOf(contact("ana")), RoutineFilter.All, now, zone, dayStart, slots = ::slots)
    ...
    fun `a contact told about and unanswered reads as a No`() {
        val row = buildRoutinesState(
            ...
            RoutineFilter.All, now, zone, dayStart, slots = ::slots,
        ).rows.single()
    ...
    fun `the kind chips are offered only when somebody is of that kind`() {
        val work = buildRoutinesState(listOf(contact("ana")), RoutineFilter.All, now, zone, dayStart, slots = ::slots)
        ...
        val filtered = buildRoutinesState(
            ...
            RoutineFilter.Kind(ContactKind.PERSONAL), now, zone, dayStart, slots = ::slots,
        )
    ...
    fun `routines and contacts share the one list`() {
        val state = buildRoutinesState(
            listOf(routine("plants"), contact("ana")), RoutineFilter.All, now, zone, dayStart, slots = ::slots,
        )
        assertEquals(setOf("plants", "ana"), state.rows.map { it.id }.toSet())
        assertEquals(2, state.total)
        assertEquals(1, state.overdue, "only the routine is overdue; the contact is queued")
    }
}
