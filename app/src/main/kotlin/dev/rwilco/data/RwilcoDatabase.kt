...
abstract class RwilcoDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATIONS: Array<Migration> = arrayOf(
            ...
            // v12: when a routine was last asked whether it had been done. Null on every
            // existing row: never asked, which is what every row written before routines is.
            object : Migration(11, 12) {
                override fun migrate(db: SupportSQLiteDatabase) {
                    db.execSQL("ALTER TABLE reminder ADD COLUMN askedAt INTEGER")
                }
            },
            // v13: when a pause was last lifted. Null on every existing row: never paused, so
            // nothing that has already rung is un-silenced by the upgrade itself.
            ...
            // v15: which kind of contact a routine is, which is what makes it one at all.
            // Null on every existing row, and that is the truth about them: nothing written
            // before this existed belongs to anybody, so every routine stays an ordinary one.
