...
class SnoozeBoardTest {
    ...
    fun `with nothing hidden the alert is exactly what it was`() {
        ...
        assertEquals(Snooze.entries.toList(), board.shown, "every answer, in the order it always had")
        assertEquals(emptyList<Snooze>(), board.more)
    ...
    fun `what is hidden waits behind the other door, the most used first`() {
        ...
        assertEquals(listOf(Snooze.TEN_MINUTES, Snooze.CUSTOM, Snooze.TWO_HOURS, Snooze.TOMORROW_MORNING), board.shown)
        // By use, and between two never used (or used alike) the order they always had.
        assertEquals(listOf(Snooze.NEXT_WEEK, Snooze.WEEKEND, Snooze.TOMORROW), board.more)
    ...
    fun `a name this build does not know hides nothing and counts for nothing`() {
        ...
        assertEquals(Snooze.entries.toList(), snoozeBoard(settings).shown)
    ...
    fun `hiding and showing is one switch per offer, and everything may be hidden`() {
        ...
        assertEquals(emptyList<Snooze>(), snoozeBoard(all).shown)
        assertEquals(Snooze.entries.toList(), snoozeBoard(all).more)
    ...
    fun `a snooze lands where the person's own terms say`() {
        val terms = AppSettings(snoozeCustomMinutes = 45).snoozeTerms
        assertEquals(Fixtures.now.plusSeconds(45 * 60), Snooze.CUSTOM.until(Fixtures.now, Fixtures.zone, terms))
        assertEquals(
            Snooze.WEEKEND.until(Fixtures.now, Fixtures.zone, terms.weekendDay, terms.weekendTime, terms.dayStart, terms.customMinutes),
            Snooze.WEEKEND.until(Fixtures.now, Fixtures.zone, terms),
        )
    }
}
