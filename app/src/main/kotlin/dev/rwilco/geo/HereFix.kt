...
import dev.rwilco.model.HERE_FIX_MAX_ACCURACY_M
 ...
 */
suspend fun hereFix(context: Context, store: PlaceWatchStore, now: Instant): Fix? {
    store.read().lastFix?.takeIf { it.speaksForHere(now) }?.let { return it }
    val found = currentLocation(context) as? LocationFix.Found ?: return null
    val location = found.location
    val accuracy = if (location.hasAccuracy()) location.accuracy.toDouble() else 0.0
    if (accuracy > HERE_FIX_MAX_ACCURACY_M) return null
    return Fix(location.latitude, location.longitude, accuracy, Instant.ofEpochMilli(location.time))
}
